...
Run `figma-fetch <url>` to materialize the requested file or node. Read `content.md` first, then `content.json` or `raw/nodes.json` only when more detail is needed. When a rendered node image exists under `assets/`, inspect it as visual evidence.
